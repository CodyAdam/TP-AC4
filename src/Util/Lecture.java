 ...
 * <b>idf</b> : identifiant qui désigne un <em>fichier ouvert en
 * lecture</em><br>
 * <ul>
 * <li>si ce paramètre est présent, la lecture se fait <em>depuis un
 * fichier</em>, et
 * dans ce cas <em>la fin du fichier <b>idf</b> ne doit pas être atteinte</em>
 * </li>
...
public class Lecture {
  ...
  public static InputStream ouvrir(String nomFich) {
    InputStream idf;
    try {
      idf = new DataInputStream(new FileInputStream(nomFich));
    } catch (IOException e) {
      idf = null;
    }
    return idf;
  ...
  public static boolean finFichier(InputStream idf) {
    try {
      return (idf != System.in && idf.available() == 0);
    } catch (IOException e) {
  ...
  /**
   * lecture dans un fichier d'un réel double éventuellement précédé par des
   * espaces et suivi
   * d'un espace.
   * 
   * @return le réel lu
  ...
  /**
   * lecture au clavier d'un réel double éventuellement précédé par des espaces et
   * suivi
   * d'un espace.
   * 
   * @return le réel lu
