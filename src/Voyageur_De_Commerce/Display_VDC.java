...
class Display_VDC extends JFrame {
	...
	// constructeur (affiche le parcours correspondant à Ind + le numéro de chaque
	// ville)
	...
	// met à jour le parcours actuel par celui correspondant à Ind (et affiche le
	// numéro des villes)
...
class Canvas_VDC extends Canvas {
	...
	public void CalculeDistance() {
		...
		for (int i = 0; i < iNbCities - 1; i++) {
			distance += Math.sqrt((CitiesPosX[Parcours[i]] - CitiesPosX[Parcours[i + 1]])
					* (CitiesPosX[Parcours[i]] - CitiesPosX[Parcours[i + 1]])
					+ (CitiesPosY[Parcours[i]] - CitiesPosY[Parcours[i + 1]])
							* (CitiesPosY[Parcours[i]] - CitiesPosY[Parcours[i + 1]]));
		}
		distance += Math.sqrt((CitiesPosX[Parcours[iNbCities - 1]] - CitiesPosX[Parcours[0]])
				* (CitiesPosX[Parcours[iNbCities - 1]] - CitiesPosX[Parcours[0]])
				+ (CitiesPosY[Parcours[iNbCities - 1]] - CitiesPosY[Parcours[0]])
						* (CitiesPosY[Parcours[iNbCities - 1]] - CitiesPosY[Parcours[0]]));
